...
fn to_calendar_day(
    ...
    calendar_day {
        ...
        is_selected: month == display_month && day_num == selected_day,
...
fn build_calendar_month(
    ...
    for i in 0..leading {
        ...
        days.push(to_calendar_day(solar, month, selected_day, today));
    }
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let state = Rc::new(RefCell::new((today.0, today.1, today.2)));
    {
        let (year, month, selected_day) = *state.borrow();
        ui.set_month_data(build_calendar_month(year, month, selected_day, today));
    }

    {
        ...
        ui.on_prev_month(move || {
            if let Some(ui) = weak.upgrade() {
                ...
                let m = SolarMonth::from_ym(s.0, s.1).next(-1);
                s.0 = m.get_year();
                s.1 = m.get_month();
                s.2 = 1;
                ui.set_month_data(build_calendar_month(s.0, s.1, s.2, today));
            }
        });
    }
...

    {
        let weak = ui.as_weak();
        let state = Rc::clone(&state);
        ui.on_day_clicked(move |day| {
            if let Some(ui) = weak.upgrade() {
                let mut s = state.borrow_mut();
                s.2 = day.max(1) as usize;
                ui.set_month_data(build_calendar_month(s.0, s.1, s.2, today));
            }
        });
    }
